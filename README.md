...
The home page game listing can be filtered by publisher and by one or more categories. Filters are applied in the browser to the statically rendered catalog; selecting multiple categories matches games in any selected category, and filter selections are cleared with the **Clear filters** button.
