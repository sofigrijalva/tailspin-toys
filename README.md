...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) loads the current repository's open GitHub issues through the authenticated `gh` CLI, ranks the three most likely to need attention, and lists the rest below. Each card includes a link to the issue and a **Work on this issue** button that adds a focused request to the current Copilot session. The board is read-only and requires `gh auth status` to succeed in the local environment.
